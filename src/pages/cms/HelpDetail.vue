...
export default {
  ...
  methods: {
    ...
    getTop() {
      return this.$http.post(this.host + ":6001/uc/ancillary/more/help/page/top", {
        cate: this.cate,
        lang: this.langPram
      });
    },
    getArticle() {
      return this.$http.post(this.host + ":6001/uc/ancillary/more/help/detail", {
        id: this.id
      });
    },
    init() {
      Promise.all([this.getTop(), this.getArticle()]).then(arr => {
        if (
          ...
          window.document.title = (this.lang == "zh_CN" ? "帮助 - " : "Help - ") + this.article.title + " - CoinPro | Global digital currency trading platform";
        } else {
    ...
    getTopList() {
      this.$http
        .post(this.host + ":6001/uc/ancillary/more/help/page/top", {
          cate: this.cate,
          lang: this.langPram
        })
        .then(res => {
    ...
    getData(id) {
      this.$http
        .post(this.host + ":6001/uc/ancillary/more/help/detail", {
          id
        })
        .then(res => {
          if (res.status == 200 && res.body.code == 0) {
