...
export default {
  ...
  methods: {
    ...
    getData(){
        ...
        this.$http.post(this.host + ":6001/uc/activity/detail", param).then(res => {
        if (res.status == 200 && res.body.code == 0) {
            this.activityDetail = res.body.data;
            window.document.title = (this.lang == "zh_CN" ? "活动 - " : "Activity - ") + this.activityDetail.title + " - CoinPro | Global Bitcoin trading platform ";
            //持仓瓜分类型进度显示处理
    ...
    send(index) {
      ...
      if (index == 1) {
        if (this.formValidateAddr.email) {
          //获取邮箱code
          this.$http.post(this.host + ":6001/uc/activity/code").then(response => {
            var resp = response.body;
            if (resp.code == 0) {
        ...
        if (this.formValidateAddr.mobileNo) {
          //获取手机code
          this.$http
            .post(this.host + ":6001/uc/mobile/activity/code")
            .then(response => {
              var resp = response.body;
              if (resp.code == 0) {
                this.$Message.success(resp.message);
