<template>
  <div class="envelope">
    ...
    <p style="margin-top: -80px;margin-bottom: 80px;text-align:center;color: rgb(255, 136, 79);font-size:13px;">该红包通过币严(COIN.PRO)发出</p>
    ...
    <router-link style="font-size:14px;text-align:center;color: #EEE;margin-top:15px;text-decoration:underline;" to="/app">© COIN.PRO | 下载APP</router-link>
    </p>
...
export default {
  data() {
    return {
      spinShow: false,
      hasReceived: false,
      inviteLink: "https://www.coin.pro/envelope/",
      envelopeNo: "",
      country: "",
      ...
      hasInviteUser: false,
      inviteUserId: "***********",
      inviteUserAvatar: "../../assets/images/logo.png",
      envelopeInfo: {
        id: 0,
        name: "**********",
        totalAmount: "0.00",
        ...
        bgImage: "./assets/images/redenvelope.png",
        state: 0,
        detail: "留给未来一个暴富的可能"
      },
      formInline: {
  ...
  methods: {
    ...
    sendCode(){
      ...
      if(!reg.test(this.formInline.phone)) {
        ...
        this.$http.post(this.host + ":6001/uc/redenvelope/code", params).then(response => {
    ...
    handleSubmit(){
      ...
      this.$http.post(this.host + ":6001/uc/redenvelope/receive", this.formInline).then(res => {
    ...
    getEnvelope(){
        ...
        this.spinShow = true;
        this.$http.post(this.host + ":6001/uc/redenvelope/query", param).then(res => {
          this.spinShow = false;
          if (res.status == 200 && res.body.code == 0) {
            ...
            window.document.title = "【" + this.envelopeInfo.totalAmount + " " + this.envelopeInfo.unit + "】" + this.envelopeInfo.name + "  — BZEX(WWW.CoinPro.COM)交易所"
            this.getEnvelopeDetailList();
          } else {
              this.$Message.error(res.body.message);
          }
    ...
    getEnvelopeDetailList(){
      ...
      this.$http.post(this.host + ":6001/uc/redenvelope/query-detail", this.queryDetail).then(res => {
        if (res.status == 200 && res.body.code == 0) {
