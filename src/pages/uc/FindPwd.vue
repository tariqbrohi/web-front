...
export default {
  ...
  methods: {
    ...
    emailReset() {
        ...
        this.$http.post(this.host + ":6001/uc/reset/email/code", params).then(response => {
            this.countdown = 60;
    ...
    afterValidate() {
      ...
      if (emailReg.test(this.formInline.user)) {
        ...
        params["geetest_validate"] = this._captchaResult.geetest_validate; //极验验证二次验证表单数据 validate
        params["geetest_seccode"] = this._captchaResult.geetest_seccode; //极验验证二次验证表单数据 seccode
        this.$http.post(this.host + ":6001/uc/mobile/reset/code", params).then(response => {
            var resp = response.body;
            if (resp.code == 0) {
              this.settime();
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          if (emailReg.test(this.formInline.user)) {
            ...
            this.$http
              .post(this.host + ":6001/uc/reset/login/password", params)
              .then(response => {
                this.countdown = 60;
                var resp = response.body;
                if (resp.code == 0) {
