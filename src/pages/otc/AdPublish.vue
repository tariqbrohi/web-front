        ...
        <div class="send-box">
          <div class="title-box">
            ...
            <p>{{$t('otc.publishad.msg1')}}
              <router-link to=":6006/otc/trade/usdt">{{$t('otc.publishad.tradead')}}</router-link>
              。
            </p>
...
export default {
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (this.isId) {
            this.$http
              .post(this.host + ":6006/otc//advertise/update", isIdparams)
              .then(response => {
            ...
            //创建
            this.$http
              .post(this.host + ":6006/otc//advertise/create", params)
              .then(response => {
                var resp = response.body;
                if (resp.code == 0) {
                  debugger;
    ...
    getAreas() {
      this.$http.post(this.host + ":6001/uc/support/country").then(response => {
        var resp = response.body;
    ...
    getMember() {
      ...
      //
      //
      // this.$http.post(this.host + ':6001/uc/approve/security/setting').then(response => {
      //     var resp = response.body;
      //     if (resp.code == 0) {
      //         if (resp.data.realName == null || resp.data.realName == "") {
      //             this.$Message.success(this.$t('otc.publishad.submittip1'));
      //             self.$router.push(':6001/uc/safe');
    ...
    getAccount() {
      ...
      let self = this;
      this.$http
        .post(this.host + ":6001/uc/approve/account/setting")
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
            if (
    ...
    getDetailAd() {
      this.isId = true;
      this.$http
        .post(this.host + ":6006/otc//advertise/detail", { id: this.$route.query.id })
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
  ...
  created() {
    ...
    let lv = (1 + this.form.premisePrice / 100).toFixed(4);
    //获取币种
    this.$http.post(this.host + ":6006/otc//coin/all").then(response => {
      var resp = response.body;
      if (resp.code == 0) {
