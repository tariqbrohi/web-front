<template>
  <div>
    <div id="fullpage">
      <div style="background-image: linear-gradient( 135deg, #F0A70A 10%, #0D25B9 100%);text-align: center;height:30px;line-height:30px;letter-spacing: 1px;">In Progress...</div>
      ...
      <div style="width: 100%;background: #141e2c;display:none;">
        <router-link to="/announcement/118930" target="_blank" style="width: 100%;">
          <img src="../../assets/images/logo.png" style="width: 72%;margin-left: 14%;margin-top: 20px;">
        </router-link>
      </div>
...
export default {
  ...
  methods: {
    ...
    getCNYRate() {
      this.$http
        .post(this.host + ":6004/market/exchange-rate/usd-cny")
        .then(response => {
          var resp = response.body;
    ...
    loadPicData() {
      ...
      param["lang"] = this.langPram;
      this.$http
        .post(this.host + ":6001/uc/ancillary/system/advertise", param)
        .then(response => {
    ...
    loadTrendData(){
      this.$http
        .post(this.host + ":6004/market/btc/trend", {})
        .then(response => {
          var resp = response.body;
